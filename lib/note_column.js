...
/*
When thinking about how to implement voices, it turns out that note columns are not going to
be enough. Inside a note column, there is a grouping, which has a minimum of one (max 4?)
A note grouping does not represent a voice, but the voicing inside a note column
The grouping is what the beaming needs in order to attach to.

The grouping will not do the stacking, but just serve as a way to attach the beam
 */
...
Presto.NoteColumn = Presto.Column.extend({
  ...
  /**
   * This computed property checks whether this note column should be part of a beam.
   * This depends on whether the lengths of any of the notes (in combination with the not yet implemented
   * voice system) is 8 or higher, and the notes themselves indicate (somehow) that they don't want to
   * be part of a beam.
   * @return {[type]} [description]
   */
  validForBeam: function () {
