...
Presto.Score = Presto.Object.extend({
  ...
  parse: function (notation) {
    ...
    if (!staffDistance) this.staffDistance = staffDistance = 10 * size;
    var vOffset = 4 * size;
    var s = this._staffs = notation.staffs.map(function (s, i) {
   ...
   */
  render: function () {
    var absPos = this._rootGrob.render(0, this.get('size') * 10 + 1);
