...
import { getEach, PrestoBase, pt2px, setEach, warn } from './tools.js';
import { initFontInfo } from './font_info.js';
...
export class Score extends PrestoBase {
   ...
   */
    ...
    _adjustStaffSpacing () {
        ...
        staffs.forEach((s, i) => {
            ...
            var maxTop = s.maximumTopOffset;
            var maxBottom = s.maximumBottomOffset;
            ...
            if (nextStaff) { // check whether the center of the next staff is far enough away to
                ...
                diff = (nextStaff.y - Math.abs(nextStaff.maximumTopOffset)) - (s.y + maxBottom);
                if (diff < 0) {
   ...
   */
    fontIsReady () {
        initFontInfo(this._ctx, this.fontSize);
        this.fontReady = true;
   ...
   */
    static from (canvas, opts = {}) {
        if (!opts.canvas) opts.canvas = canvas;
        var s = Score.create(opts);
        this._scores.push(s);
        return s;
    }
   ...
   */
    static fontActive () {
        if (Score._fontIsActive) return; // don't run twice
