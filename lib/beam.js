...
27-11-2015
After reading the above after a while it just doesn't seem possible to use option 1. As the last remark before already sort
of indicates, beams are actually between notes of a voice, not between note columns.
In a way it seems that the best solution is a merge of option 1 and 2. It is true that a beam as an object is closer to the staff
in hierarchy than a note column, but at the same time it needs to attach to notes, which are part of the note columns.
In the Presto Object hierarchy a voice currently does not exist.

So, in a way we need to do a kind of a subscription model, but one where the staff checks how many voices a note colum has,
creates beam objects accordingly (one for every voice) when necessary, and checks whether the notes should be part of a beam based
on the position in the bar and the length of the note, or whether the notes themselves indicate whether they should be part of a
beam. (this allows for implementation of the Lilypond beaming syntax), and subscribe the notes.
