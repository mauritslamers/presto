...
Presto.Array = function() {
  ...
  if (arguments.length > 0) {
    if (arguments.length === 1 && Array.isArray(arguments[0])) {
      ret = Array.apply(ret, arguments[0]);
    }
    else Array.apply(ret, arguments);
  }
...
Presto.mixin(Presto.Array.prototype, Presto.Reducers, {
  ...
  */
  getEach: function (key) {
    return Presto.Array.create(this.map(function (next) {
...
Presto.mixin(Presto.Array, {
  create: function (opts) {
    var c = this;
    // the check for opts prevents having an undefined value as first element in the new array
    if (opts) return new c(opts);
    else return new c();
  }
});
