...
Presto.Note = Presto.Grob.extend({
  ...
  addStem: function () {
    ...
    if (stemUp) pos *= -1; // does this work?

    if (pos > 4) {
      stemLength = 4 * staffSpace;
    }
    else if (pos <= 4 && pos >= 0) {
      stemLength = 6 * staffSpace + (staffSpace / (-5+pos)); // using -5, to make it smaller as the absPos increases
    }
    else if (pos < 0 && pos >= -7) {
      stemLength = 6 * staffSpace;
    }
    else {
      stemLength = this.staff.calculateVerticalOffsetFor(0) - this.staff.calculateVerticalOffsetFor(pos);
    }

    var startX = stemUp ? (this._noteHeadWidth / 2) - 3: this.x; // static value, for now until we draw
