...
import { Stem, STEMDIRECTION_DOWN, STEMDIRECTION_UP } from './stem.js';
...
export const distanceBetween = function (noteOne, noteTwo) {
    ...
    var octDist = noteOne.octave - noteTwo.octave;
...
//if (noteOne.get('noteId') === noteTwo.get('noteId')) return 1; // fast path for primes
    var dist = distanceBetween(noteOne, noteTwo); // solve the off by one for intervals
    if (dist > 0) dist += 1;
    if (dist < 0) dist -= 1;
    if (dist === 0) {
...
export const isValidNoteName = function (name) {
    ...
    var exts = ['is', 'es', 's', 'eses', 'isis', 'sas', 'ses'], ext;
    if (name) {
...
export class Note extends Grob {
   ...
   */
    ...
    set stemUp (val) {
        if (val === true) this.stemDirection = STEMDIRECTION_UP;
    }
    ...
    set stemDown (val) {
        if (val === true) this.stemDirection = STEMDIRECTION_DOWN;
    }
    ...
    /**
     * Color with which the note should be drawn
     * @type {string}
     */
    color = 'black';
