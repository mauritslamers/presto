...
// cache for font info. Uses the fontsize as key
// shape:
// { [fontSize]: { info, metrics }
const fetaFontInfoCache = {};

export function fontInfoFor (fontSize) {
    return fetaFontInfoCache[fontSize].info;
}

export function fontMetricsFor (fontSize) {
    return fetaFontInfoCache[fontSize].metrics;
}

export function initFontInfo (canvasCtx, fontSize) {
    if (fetaFontInfoCache[fontSize]) return; // already exists
    const fontInfo = {
        info: {},
        metrics: {}
    };
    canvasCtx.font = fontSize + 'pt Emmentaler26';
    for (let key of Object.keys(fetaFontInfoCodes)) {
        const code = fetaFontInfoCodes[key];
        const val = String.fromCharCode(code);
        fontInfo.info[key] = val;
        fontInfo.metrics[key] = canvasCtx.measureText(val);
    }
    fetaFontInfoCache[fontSize] = fontInfo;
}

export const fetaFontInfoCodes = {
    'rests.0' : 0xE100,
    'rests.1' : 0xE101,
    'rests.0o' : 0xE102,
    'rests.1o' : 0xE103,
