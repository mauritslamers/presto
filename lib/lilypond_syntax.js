...
import { noteNames as PrestoNoteNames } from "./note.js";

export const lilypondParser = {
   ...
   */
    ...
    _markupBlockShortHandRegex: /\^[\s\S]*?\"(.*?)\"/,
   ...
   */
    parseVoice: function (voiceContent) {
        ...
        var findSpacer = function (voice, fromIndex) {
            ...
            var spacerPos = [' ', '\n', '\t', '~', '\\', '^', '_'].reduce(function (reduceVal, item) {
        ...
        for (var i = 0; i < len; i += 1) {
            ...
            if (curItem === '^') {
                match = this._markupBlockShortHandRegex.exec(v.slice(i));
                if (match) {
                    i += match[0].length - 1;
                    continue;
                }
            ...
            if (curItem === '\\') { // command
                ...
                if (next === -1) {
                    if (v.indexOf('\n', i) === -1) {
                        throw new Error('Syntax error: no space after command?');
                    }
                    else {
                        next = v.indexOf('\n', i);
                    }
   ...
   */
    parseChord: function (chord, reference, reflength) {
        ...
        notes.split(' ').forEach(function (n) {
            ...
            if (n) {
                n.length = length; // overwrite standard length
