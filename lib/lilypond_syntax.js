...
Presto.lilypondParser = {
  ...
  parseVoice: function (voiceContent) {
    ...
    var ret = [];
    var len = v.length;
    var curItem, prev, match, cmd, chord, next, note;
    ...
    //     continue;
    //   }
    //   if (curItem.indexOf("relative") > -1) { // parse relative and set prev
    //     match = this._relativeRegex.exec(voiceContent.slice(contentIndex));
    //     prev = this.parseNote(match[1] + match[2]);
    //     while (!inRelative) {
    //       i += 1;
    //       curItem = v[i];
    //       contentIndex += curItem.length + 1;
    //       indexOfOpen = curItem.indexOf("{");
    //       indexOfClose = curItem.indexOf("}");
    //       inRelative = indexOfOpen > -1 && indexOfClose === -1; // { but not } means we are in the relative
    //       if (!inRelative && indexOfClose > -1) {
    //         if (indexOfClose - indexOfOpen > 1) { // for some reason we have something like {a'2}
    //           throw new Error("Syntax error: " + match[3]);
    //         }
    //       }
    //     }
    //   }
    //   if (inRelative) {
    //     indexOfClose = curItem.indexOf("}");
    //     if (indexOfClose === 0) { // just a single "}"
    //       inRelative = false;
    //       prev = null;
    //       contentIndex += 2;
    //       continue;
    //     }
    //   }
    //   // now parse the note, first check for chords
    //   indexOfOpen = curItem.indexOf("<");
