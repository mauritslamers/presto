import { Barline, T_SINGLE } from './barline.js';
import { Grob } from './grob.js';
import { Line } from './line.js';
import { distanceBetween, Note } from './note.js';
...
export class Staff extends Grob {
    ...
    calculateVerticalPositionFor (notename, octave) {
        ...
        var cnote = this.clefNote;
        var cpos = this.clefPosition;
    ...
    addTimeSignature () {
        ...
        this.addChildGrob(c);
        this._currentX += c.width + (4 * staffSpace);
        return this;
    }
    ...
    addKeySignature () {
        ...
        if (key) {
            ...
            keySig.forEach(function (k) {
                ...
                this.addChildGrob(symbol);
                this._currentX += symbol.width;
            }, this);
            this.keySignature = keySig;
        }
        else {
            this.keySignature = CLEFS[this.clef].keySignatures.c;
        }
    ...
    _addNoteEventToNotationCache (noteEvent) {
        ...
        if (curLength) { // ignore no-length events
            ...
            try {
                ...
                throw new Error('Presto.Staff: the value of the note being parsed is smaller than the cursorSize on Presto.Score, please increase the cursorSize.');
            }
        }
    }   
    ...
    checkAndDrawBarline () {
        ...
        if (cursor - prevBarAt === numCursorsPerBar) {
            ...
            this.addChildGrob(Barline.create({
                x: this._currentX,
