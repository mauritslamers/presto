...
Presto.Staff = Presto.Grob.extend({
  ...
  addClef: function () {
    ...
    this._currentX += symbol.get('width') / 2; // no clue why a clef is so wide...
  },
  ...
  _generateNotationCache: function () {
    ...
    n.forEach(function (noteEvent) {
      if (Array.isArray(noteEvent)) {
        ...
        curLength = noteEvent.map(this._calculateDottedLength).get('@max'); // the smallest note has the biggest number
      }
