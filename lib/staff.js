...
Presto.Staff = Presto.Grob.extend({
  ...
  setVerticalOffsetFor: function (note) {
    ...
    if (keySig) {
      // we need to check whether the root name of the note matches one of the accidentals of the keySig
      // if yes, double check whether the
  ...
  addKeySignature: function () {
    ...
    if (key) {
      ...
      keySig.forEach(function (k) {
        ...
        var n = Presto.Note.create({ name: name, octave: oct, length: 4, staff: this, score: this.score });
        this.setVerticalOffsetFor(n);
        n.update();
        var acc = n.get('accidental');
        acc.x = this._currentX;
        acc.y = n.y + 2; // why the offset?
        this.addChildGrob(acc);
        this._currentX += acc.get('width');
      }, this);
    ...
    this.resetAccidentals(); // set the list with current accidentals
  },
   ...
   */
  resetAccidentals: function () {
    var names = Presto.Note._noteNames;
    var keySig = this.get('keySignature');
    var l = {};
