...
Presto.Staff = Presto.Grob.extend({
   ...
   */
  _calculateVerticalOffsetFor: function (pos) {
  ...
  /**
   * Calculate the vertical offset for a note object and set it on the note object.
   * This also sets the number of helper lines on the note.
   * Could be replaced by a predefined list and then have a lookup, but this is more flexible
  ...
  setVerticalOffsetFor: function (note) {
    ...
    note.y = this._calculateVerticalOffsetFor(notePos);
    var i;
    var helperLines = Presto.Array.create();
    if (notePos > 5) {
      for (i = 6; i <= notePos; i += 2) {
        helperLines.push({ y: note.y - this._calculateVerticalOffsetFor(i) });
      }
